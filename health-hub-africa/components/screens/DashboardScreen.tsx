    ...
    <div className="flex flex-col gap-6 pb-20 md:pb-5">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className={overviewView === 'grid' ? 'grid grid-cols-1 sm:grid-cols-3 gap-3.5' : 'flex flex-col gap-2.5'}>
          ...
          <div className={overviewView === 'grid'
            ? 'p-5 rounded-[22px] border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm flex flex-col gap-2.5'
            : 'p-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm flex items-center justify-between gap-3'
          }>
          ...
          <div className={overviewView === 'grid'
            ...
            {nextAppt ? (
              <>
                ...
                <p className="text-[11px] text-[var(--color-text-muted)] font-extrabold uppercase tracking-wider shrink-0">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Heart rate card */}
          <div className="p-5 rounded-[24px] border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm flex flex-col justify-between min-h-[190px]">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-extrabold text-[var(--color-text-muted)] uppercase tracking-wider">Heart rate</span>
              <label className="relative flex items-center gap-1.5 text-[9px] font-extrabold uppercase tracking-wider text-[var(--color-text-muted)] hover:text-[var(--color-text)] border border-[var(--color-border)] rounded-full px-3 py-1 bg-[var(--color-bg)]/50 transition-colors cursor-pointer">
                <Clock size={10} />
          ...
          <div className="p-5 rounded-[24px] border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm flex flex-col justify-between min-h-[190px]">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-extrabold text-[var(--color-text-muted)] uppercase tracking-wider">Sleeping periodic</span>
              <label className="relative flex items-center gap-1.5 text-[9px] font-extrabold uppercase tracking-wider text-[var(--color-text-muted)] hover:text-[var(--color-text)] border border-[var(--color-border)] rounded-full px-3 py-1 bg-[var(--color-bg)]/50 transition-colors cursor-pointer">
                <Calendar size={10} />
      ...
      <Card className="rounded-[24px]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {/* CTA impressions (spec §8.3) — pairs with the ui_click these chips
              ...
              Followed up on Labs' "Book CareTest™" CTA, and on the primary
              CTAs in Appointments/TeleCare/Payments/Subscriptions — still
              open: Upload/Share record controls, per-result lab actions,
              vitals entry/device-sync, nav menu items, profile-completion
              prompts, search. */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="rounded-[24px] p-5">
          <CardTitle className="text-xs font-extrabold text-[var(--color-text-muted)] uppercase tracking-wider mb-3">Scheduled Care</CardTitle>
          {nextAppt ? (
            <div className="flex flex-col gap-3">
              ...
              <p className="text-lg font-extrabold text-[var(--color-text)]" style={{ fontFamily: 'var(--font-display)' }}>
                {formatDate(nextAppt.scheduledAt)}
              </p>
              {nextAppt.provider && (
        ...
        <Card className="rounded-[24px] p-5">
          <CardTitle className="text-xs font-extrabold text-[var(--color-text-muted)] uppercase tracking-wider mb-4">Your Doctor</CardTitle>
          {nextAppt?.provider ? (
            <>
