...
export async function POST(req: NextRequest) {
  ...
  // Patients don't use 2FA today, but pass the flag through unharmed if the backend ever sets it.
  ...
  // Access token — readable by JS so lib/api.ts can attach Authorization: Bearer.
  ...
  // Refresh token — HttpOnly: JS cannot read it; only the /api/auth/refresh route can.
