...
import { FlaskConical, Building2, Home, BarChart3, Stethoscope, RefreshCw } from 'lucide-react'
import type { LucideIcon } from 'lucide-react'


export const metadata: Metadata = {
  title: 'CareTest™ — Lab Tests & Diagnostics | MyHealth Vault+™',
  description:
    'Book 200+ diagnostic tests and lab work at certified partner labs. Results flow directly into your MyVault™ health record.',
}

const features: Array<{ icon: LucideIcon; title: string; desc: string }> = [
  {
    icon: FlaskConical,
    title: '200+ Diagnostic Tests',
    desc: 'From full blood counts to specialised markers; every test bookable online in seconds.',
  },
  {
    icon: Building2,
    title: 'Certified Lab Network',
    desc: 'Tests conducted at CAC-registered, quality-assured partner labs across Nigeria.',
  },
  {
    icon: Home,
    title: 'Home Sample Collection',
    desc: 'Available in Lagos and Abuja; a trained, certified collector visits your location at your chosen time.',
  },
  {
    icon: BarChart3,
    title: 'Results in Your Vault',
    desc: 'Lab results are uploaded as structured data and PDF directly to your MyVault™ health record.',
  },
  {
    icon: Stethoscope,
    title: 'Result Interpretation',
    desc: 'Book a TeleCare™ consultation directly from your results page to discuss findings with a clinician.',
  },
  {
    icon: RefreshCw,
    title: 'FHIR-Compliant Data',
    desc: 'All results stored in a globally interoperable format, shareable with any clinician instantly.',
  },
]
...
const steps = [
  {
    num: '01',
    title: 'Select Your Tests',
    desc: 'Browse the full CareTest™ catalogue. Search by name, condition, or recommended panel.',
  },
  {
    num: '02',
    title: 'Choose Collection',
    desc: 'Visit a partner lab or book a home collection. Select your preferred date and time.',
  },
  {
    num: '03',
    title: 'Provide Your Sample',
    desc: 'At the lab or at your door, a certified collector processes your sample with full chain-of-custody.',
  },
  {
    num: '04',
    title: 'Results in Your Vault',
    desc: 'Results appear in your MyVault™ record within 24–72 hours with automatic notifications.',
  },
]
...
const planRows = [
  ...
  { plan: 'Enterprise', access: 'Priority processing', highlight: false },
  { plan: 'Corporate & HMO', access: 'Bulk allocation', highlight: false },
]

const popularTests = [
  'Full Blood Count (FBC)', 'Lipid Profile', 'Liver Function Test', 'Kidney Function Test',
  'HbA1c (Diabetes Screen)', 'Thyroid Panel (TSH, T3, T4)', 'PSA (Prostate)',
  'HIV Screening', 'Hepatitis B & C', 'Malaria RDT', 'Urinalysis', 'Chest X-Ray Review',
]

export default function CareTestPage() {
  return (
    <div style={{ width: '100%', overflowX: 'hidden', background: '#F1F4EF' }}>
      ...
      <div style={{ margin: '16px 16px 24px', borderRadius: 28, overflow: 'hidden' }}>
        ...
        >
          <Image
            src="https://images.unsplash.com/photo-1579684385127-1ef15d508118?w=1920&h=1080&fit=crop&q=85"
            alt="Lab diagnostics"
            fill
            priority
      ...
      <div style={{ margin: '0 24px 24px', borderRadius: 28, overflow: 'hidden', background: '#F7FAF7' }}>
        <section style={{ maxWidth: 1280, margin: '0 auto', padding: '88px 56px' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-space-grotesk), sans-serif', fontWeight: 600, fontSize: 'clamp(28px, 3.5vw, 40px)', lineHeight: 1.1, letterSpacing: '-0.02em', margin: '0 auto', maxWidth: 520, color: '#07251C' }}>
              Commonly booked{' '}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
            {popularTests.map((test) => (
              <span key={test} style={{ background: '#fff', border: '1px solid rgba(7,37,28,0.1)', borderRadius: 100, padding: '10px 20px', fontSize: 14, fontWeight: 500, color: '#07251C' }}>
                {test}
              </span>
            ))}
