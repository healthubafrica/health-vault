      ...
      <div style={{ margin: '16px 16px 24px', borderRadius: 28, overflow: 'hidden' }}>
        <section style={{ position: 'relative', background: '#041E14', overflow: 'hidden', minHeight: '88vh', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 1100, margin: '52px auto 0', perspective: '1000px', height: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
            {[
              { rotate: -12, translateY: -12, scale: 0.88, zIndex: 2, offsetX: -10, bg: '#fff', w: 185, inner: (<div style={{ padding: 16 }}><div style={{ fontSize: 9, fontWeight: 700, color: '#0E8567', marginBottom: 10 }}>GROUP ENROLMENT</div><div style={{ fontSize: 12, fontWeight: 600, color: '#07251C', marginBottom: 6 }}>From 10 members</div>{['Full portal access', 'Priority services', 'Usage reporting'].map((f) => (<div key={f} style={{ fontSize: 10, color: '#41584E', marginBottom: 4 }}>✓ {f}</div>))}</div>) },
              { rotate: 0, translateY: 0, scale: 1, zIndex: 5, offsetX: 0, bg: '#07251C', w: 200, inner: (<div style={{ padding: 18 }}><div style={{ fontSize: 9, fontWeight: 700, color: '#34E0A0', marginBottom: 8 }}>CORPORATE</div><div style={{ fontSize: 16, fontWeight: 600, color: '#fff', lineHeight: 1.3 }}>Tailored healthcare benefits for your organisation</div></div>) },
              { rotate: 12, translateY: -12, scale: 0.88, zIndex: 2, offsetX: 10, bg: '#07251C', w: 175, inner: (<div style={{ padding: 16 }}><div style={{ fontSize: 9, fontWeight: 700, color: '#34E0A0', marginBottom: 8 }}>HMO INTEGRATION</div><div style={{ fontSize: 12, fontWeight: 600, color: '#fff' }}>Compatible with major HMO providers</div></div>) },
            ].map((card, i) => (
          ...
          <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '28px 32px 52px' }}>
            <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 13.5, margin: 0 }}>
              Trusted by employers, HMOs, estates, schools, and government institutions.
            </p>
          </div>
        </section>
      </div>
