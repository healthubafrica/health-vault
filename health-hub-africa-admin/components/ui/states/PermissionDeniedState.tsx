...
export function PermissionDeniedState({
  requiredRole = 'Chief Medical Officer / Super Admin',
  currentRole = 'Junior Clinical Staff (Read-Only)',
  onRequestAccess,
  onGoBack,
  className = '',
  ...
  return (
    ...
    >
      ...
      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[var(--color-bg,#F4F6F5)] text-[11px] font-bold text-[var(--color-text-muted)] border border-[var(--color-border)] mb-3">
        <UserCheck className="w-3.5 h-3.5 text-[var(--color-primary,#137333)]" />
      ...
      <p className="text-xs text-[var(--color-text-muted)] max-w-xs mb-5 leading-relaxed">
        You don't have permission to modify clinical override options or emergency dispatch controls.
      </p>

      <div className="w-full p-3.5 rounded-2xl bg-[var(--color-bg,#F4F6F5)] border border-[var(--color-border)] mb-6 text-left space-y-1">
        <p className="text-[10px] font-bold text-[var(--color-text-faint,#9CA3AF)] uppercase tracking-wider">Required Privilege Level:</p>
      ...
      {requestSent ? (
        <div className="w-full p-3.5 rounded-2xl bg-emerald-500/10 text-emerald-600 text-xs font-semibold flex items-center justify-center gap-2">
          <CheckCircle2 className="w-4 h-4" /> Access request submitted to Medical Director.
        </div>
      ) : (
        <div className="flex flex-wrap items-center justify-center gap-2.5">
          ...
          <Button size="sm" onClick={() => setShowModal(true)} className="gap-1.5 bg-[var(--color-primary,#137333)] text-white">
          ...
          >
            ...
            <form onSubmit={handleRequestSubmit} className="space-y-3">
              <textarea
                ...
                className="w-full p-3 text-xs rounded-xl bg-[var(--color-bg,#F4F6F5)] border border-[var(--color-border)] text-[var(--color-text)] focus:border-[var(--color-primary,#137333)]"
              />
              ...
              <div className="flex items-center justify-end gap-2">
                ...
                <Button type="submit" size="sm" disabled={isSubmitting} className="bg-[var(--color-primary,#137333)] text-white">
